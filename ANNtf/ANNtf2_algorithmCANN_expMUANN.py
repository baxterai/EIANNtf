# -*- coding: utf-8 -*-
"""ANNtf2_algorithmCANN_expMUANN.py
...
Define fully connected multi propagation (per layer; with synaptic delta calculation) update artificial neural network (CANN_expMUANN)
...
def neuralNetworkPropagationCANN_expMUANNtrain(x, y, networkIndex=1):

	if(trainUsingSingleLayerBackprop):
		neuralNetworkPropagationCANN_expMUANNtrainBackprop(x, y, networkIndex)
	else:
		neuralNetworkPropagationCANN_expMUANNtrainBio(x, y, networkIndex)

def neuralNetworkPropagationCANN_expMUANNtrainBackprop(x, y, networkIndex=1):
...
def executeOptimisationSub(x, yLayer, lTrain, networkIndex=1):
	...
	with tf.GradientTape() as g:
		pred = neuralNetworkPropagationCANNlayer(x, lTrain, networkIndex, recordAtrace=True)
...
def neuralNetworkPropagationCANN_expMUANNtrainBio(x, y, networkIndex=1):
	
	#TODO: needs to pass back through relu
	
	#print("neuralNetworkPropagationCANN_expMUANNtrain")
	
	neuralNetworkPropagationCANN(x, networkIndex=networkIndex, recordAtrace=True)

	#perform highest layer weight updates
	pred, ATop = neuralNetworkPropagationCANNlayer(x, numberOfLayers, networkIndex)
	...
	for l in reversed(range(1, numberOfLayers)):
		...
		pred, AhigherNew = neuralNetworkPropagationCANNlayer(x, l+1, networkIndex, recordAtrace=False)
